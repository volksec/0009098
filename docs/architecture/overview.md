...
A rede `pdc-data` não tem saída externa: só a `secure-api` e os `workers` alcançam o banco, e o
banco não alcança nada. O frontend nunca fala com o PostgreSQL — toda leitura passa pela API, que
é onde o contexto de tenant é aplicado.
...
compromete o isolamento — é o que os testes de integração verificam, conectando como `app_user`
sem `BYPASSRLS` e tentando alcançar dados de outro tenant.
...
| **Serilog** | Log estruturado com *enricher* de redação de dados sensíveis | Configuração inicial maior | Logging built-in: sem redação estruturada nem enrichment |
...
| **Storybook** | Documenta o design system próprio | Manutenção |
...
| **Docker Compose** | Ambiente completo em um comando | Não é orquestração de produção |
...
| "As invariantes seguram" | `dotnet test tests/integration` — cada invariante é violada de propósito contra PostgreSQL real e o banco precisa recusar |
