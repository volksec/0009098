-- =====================================================================
-- V011 — POLICY_PREMIUM_MISMATCH comparava contra o prêmio errado
-- =====================================================================
--
-- A verificação somava os prêmios das coberturas da apólice e exigia que o
-- resultado fosse igual ao prêmio TOTAL. Isso contradiz o modelo de cálculo:
--
--     prêmio líquido = Σ prêmios das coberturas
--     prêmio total   = prêmio líquido × (1 + carregamento)
--
-- O carregamento não pertence a nenhuma cobertura — é custo e tributo aplicado
-- sobre o conjunto. Exigir que a soma das partes reproduza o total é exigir que
-- o carregamento não exista.
--
-- A verificação só passava despercebida porque nunca era executada de verdade:
-- até a V010 a função rodava cega sob RLS e devolvia zero para tudo. Assim que
-- passou a enxergar, acusou a primeira apólice emitida pela aplicação — que
-- estava correta. Era a régua que estava torta, não a peça.
--
-- Esta migration corrige a régua: a soma das coberturas passa a ser comparada
-- com o prêmio líquido, que é o que o domínio realmente garante.
...
BEGIN
    ...
    UNION ALL
    -- ★ Σ das coberturas é o prêmio LÍQUIDO; o total inclui o carregamento
    SELECT 'POLICY_PREMIUM_MISMATCH', count(*)
      FROM (SELECT p.id FROM policies p
             JOIN policy_coverages c ON c.policy_id = p.id
            GROUP BY p.id, (p.net_premium).amount
           HAVING sum((c.premium).amount) <> (p.net_premium).amount) x
